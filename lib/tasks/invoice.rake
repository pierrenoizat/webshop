...
namespace :invoice do
  ...
  task :generate_pdf => :environment do # rake invoice:generate_pdf
    ...
    def display_invoice_lines(number)
    	@H = Hash.new("0") # hash to receive details for display of each line

    	@invoice = Invoice.find_by_id(number)
     ...
     def count_lines_from_string(string)
    	...
    	def generate_pdf_invoice(hache, compteur, number)
        ...
        temps = case I18n.locale.to_s
      	...
      	rate = ActionView::Helpers::NumberHelper.number_to_currency($CONV_RATE * $MARGIN_RATE, :unit => "EUR", :separator => ".", :delimiter => " ", :format => "%n %u")
    ...
    number = ask('Please insert invoice number:')
    @invoice= Invoice.find_by_id(number)
    hache = display_invoice_lines(number)
    compteur = count_lines_from_string(@invoice.contenu)
    generate_pdf_invoice(hache, compteur,number) # generates pdf invoice in tmp folder
  end # end task generate_pdf
 
end # end namespace invoice